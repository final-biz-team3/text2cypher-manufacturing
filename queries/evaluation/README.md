...
이번 PR은 RQ01~RQ20 평가기와 Gold, 그리고 미병합 브랜치를 사람이 선택해
실행하는 report-only GitHub Actions까지를 완료 범위로 한다. Actions의
...
따라서 이 workflow를 처음 추가하는 PR 자체는 로컬 검증 후 병합해야 하며, 한 번
기본 브랜치에 들어간 뒤부터 후속 PR 브랜치를 병합 전에 선택해 수동 실행할 수 있다.

다음은 후속 PR 범위다.

- 모델 SQL/Cypher 품질과 성능 개선
- canonical·robustness 반복 결과를 검토한 승인 baseline 등록
- 정기 실행, 재사용 workflow와 자동 배포 차단 gate 활성화
- RQ18~RQ20 애플리케이션 계산·최종 결합 E2E
...
종료 코드는 리포트 완료 `0`, 환경·API·DB·snapshot·Gold 오류 `2`다. 모델의 쿼리
오답은 결과표에 기록하되 이번 PR에서는 실행 자체를 실패시키지 않는다. 결과는
...
`queryPipelinePass`를 막지 않는다. entity 품질 개선과 차단 기준 편입은 후속 모델
품질 PR에서 실제 반복 결과를 보고 결정한다.
