...
매 실행마다 새 Neo4j 데이터베이스를 만들어 거기에만 적재·검증하고, 통과했을
때만 기본 데이터베이스로 승격한다(prune 기반 적재는 쓰지 않는다) - 실패해도
라이브 데이터베이스는 한 번도 안 건드려서 쓰기 도중 실패로 인한 부분 갱신이
원천적으로 불가능하다. 기존 기본 데이터베이스는 지우지 않고 멈춘 채로
남는다(사람이 확인 후 직접 정리, PostgreSQL의 {db}_previous_<timestamp>
보존과 같은 철학).

승격(retry_promote) 안전장치: Neo4j는 PostgreSQL의 ALTER DATABASE RENAME
같은 원자적 개명 명령이 없어서(STOP DATABASE + setDefaultDatabase로 흉내냄),
PostgreSQL과 같은 수준의 안전을 앱 레벨에서 직접 만들어야 한다. STOP
DATABASE는 활성 트랜잭션이 있어도 안전장치 없이 강제로 끊어버리므로(
PostgreSQL의 ObjectInUse 같은 보호가 없음), 호출 전에 SHOW TRANSACTIONS로
대상 DB의 활성 트랜잭션을 직접 확인해서 있으면 거부한다. 또한
setDefaultDatabase()에는 원자적 충돌 감지가 없어서, 두 세션이 동시에 승격을
시도하면 나중 세션이 먼저 세션이 이미 끝낸 승격을 조용히 덮어쓸 수 있다
...
def apply_constraints(driver: Driver, cypher_path: Path, *, database: str) -> None:
    ...
    먼저 `//` 주석 줄을 제거한 뒤 `;`로 나눈다 - 파일 맨 앞 주석 블록 뒤에는
    개행만 있고 세미콜론이 없어서, 주석 제거 없이 바로 `;`로 나누면 첫
