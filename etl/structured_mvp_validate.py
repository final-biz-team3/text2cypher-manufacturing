...
def find_dangling_relationship_rows(
    ...
    *,
    ...
    docs/design/3-structured_mvp_loading_rules.md 5절: "하나라도 존재하면 관계를 조용히
...
def verify_fixture_entities(driver: Driver, entities: dict[str, Any]) -> list[str]:
    ...
    docs/design/3-structured_mvp_loading_rules.md 7절)을 검사한다. Gold 쿼리 자체는 이번
...
def verify_work_order_17747_fixture(driver: Driver) -> list[str]:
    """docs/design/3-structured_mvp_loading_rules.md 7절의 구체적 fixture 검증:
...
def verify_bom_680_to_492_quantity(driver: Driver) -> list[str]:
    """docs/design/3-structured_mvp_loading_rules.md 7절: Product 680에서 492로 가는 필요수량이
