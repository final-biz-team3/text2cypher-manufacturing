...
def count_nodes_by_label(
    ...
    """라벨별 노드 건수를 센다. sync_run_id를 주면 그 실행에서 적재된 노드만 센다.

    세션을 직접 받는다 - 어느 데이터베이스를 대상으로 셀지(승격 전 새 DB vs
    기본 DB)는 호출자가 driver.session(database=...)로 정한다.
    """
    counts: dict[str, int] = {}
    for label in labels:
        result = session.run(
...
def count_relationships_by_type(
    ...
    """관계타입별 건수를 센다. session/sync_run_id 의미는 count_nodes_by_label과 동일하다."""
    counts: dict[str, int] = {}
...
def verify_fixture_entities(
    ...
    session/sync_run_id 의미는 count_nodes_by_label과 동일하다.
    """
    failures: list[str] = []
    product_checks = [
    ...
    if not _entity_exists(session, "Supplier", "supplierId", supplier_id, sync_run_id):
...
def main() -> None:
    ...
    try:
        with driver.session() as session:
            print("1) 건수 확인")
            node_counts = count_nodes_by_label(session, BUSINESS_LABELS)
            rel_counts = count_relationships_by_type(session, RELATIONSHIP_TYPES)
