...
> **2026-08-20 정정**: 아래 9번("prune은 새 적재와 참조 검증이 끝난 뒤에만 수행한다")은
> 원 설계 그대로 보존한다. 실제 구현은 prune 자체를 없애고 매 실행마다 새 Neo4j
> 데이터베이스에 적재·검증한 뒤 통과했을 때만 기본 데이터베이스로 승격하는 방식으로
> 바뀌었다 - 자세한 이유와 새 실행 순서는 `docs/etl/2-structured_mvp_loading_rules.md`
> 상단 정정 노트 참고.
...
| `docs/etl/1-structured_mvp_source_mapping.md` | PostgreSQL 테이블·컬럼 매핑 |
| `docs/etl/2-structured_mvp_loading_rules.md` | 적재 순서·멱등성·prune·검증 규칙 |
