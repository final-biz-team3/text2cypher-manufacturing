...
from strict_query.snapshot.orchestrator.query_conditions import (
    missing_numeric_filter_literals,
)
from strict_query.snapshot.orchestrator.state import QueryFailure, ToolName
    ...
    async def tools(state: RetryAgentState) -> dict:
        ...
        def failure(
            ...
            return {
                ...
                "attempts": [
                    *attempts,
                    {"query": query_text, "error": safe_failure["user_safe_reason"]},
                ],
                "retryable": retryable,
