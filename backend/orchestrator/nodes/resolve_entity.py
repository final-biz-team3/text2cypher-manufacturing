...
def make_resolve_entity_node(
    ...
    def resolve_entity(state: OrchestratorState) -> dict:
        ...
        product_name = _extract_product_name(query, openai_client)
        if product_name is None:
            logger.info("resolve_entity: query=%r -> entity=None (제품 미언급)", query)
            return {"entity": None}
        ...
        logger.info("resolve_entity: query=%r -> entity=%s", query, entity)
