...
_NUMBER_SOURCE = r"[-+]?\d+(?:[.,]\d+)*"
_COMPARISON_FILTER = re.compile(
    rf"(?P<number>{_NUMBER_SOURCE})\s*"
    r"(?:(?:원|%|퍼센트|개|건|명|대|곳|시간|분|초)\s*)?"
    r"(?P<comparison>인(?:\s+경우)?|짜리|이상|이하|초과|미만|"
    r"같(?:은|이)|보다\s*(?:큰|작은|많은|적은))"
)
_DATE_FILTER = re.compile(rf"(?P<number>{_NUMBER_SOURCE})\s*(?:년|월|일)\b")
_RANK_FILTER = re.compile(
    rf"(?:상위|하위)\s*(?P<number>{_NUMBER_SOURCE})\s*(?:개|건|명|대|곳)?"
)
_QUERY_NUMBER = re.compile(
    rf"(?<![A-Za-z0-9])(?P<number>{_NUMBER_SOURCE})(?![A-Za-z0-9])"
)
...
def required_numeric_constraints(question: str) -> list[NumericConstraint]:
    ...
    constraints = [
        NumericConstraint(
            _normalize_number(match.group("number")),
            _COMPARISON_OPERATORS[" ".join(match.group("comparison").split())],
        )
        for match in _COMPARISON_FILTER.finditer(question)
    ]
    constraints.extend(
        NumericConstraint(_normalize_number(match.group("number")), "present")
        for match in _DATE_FILTER.finditer(question)
    )
    constraints.extend(
        NumericConstraint(_normalize_number(match.group("number")), "limit")
...
def missing_numeric_filter_literals(question: str, generated_query: str) -> set[str]:
    query_numbers = list(_QUERY_NUMBER.finditer(generated_query))
    missing: set[str] = set()
    for constraint in required_numeric_constraints(question):
        candidates = [
            match
            for match in query_numbers
            if _normalize_number(match.group("number")) == constraint.value
        ]
        if not any(
