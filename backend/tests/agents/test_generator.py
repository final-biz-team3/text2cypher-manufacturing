...
def test_generate_query_returns_trimmed_response_content(
    ...
    query = generate_query(openai_client, messages)
...
def test_generate_query_rejects_response_without_choices() -> None:
    ...
    with pytest.raises(ValueError, match="LLM returned no query choices"):
        generate_query(openai_client, [])


def test_generate_query_requires_configured_model(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_generate_query_rejects_truncated_response() -> None:
...
def test_generate_query_rejects_none_content() -> None:
...
def test_generate_query_rejects_blank_content() -> None:
