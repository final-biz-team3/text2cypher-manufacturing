...
# 현재부터 72시간 전까지 3시간 간격으로 실제 코드 기반 이벤트 세트를 배치한다.
for ($slot = 0; $slot -lt 24; $slot++) {
    $baseTime = $anchor.AddHours(-($slot * 3))
    $prefix = "three-day-$($baseTime.ToString('MMdd-HHmm'))"
    $seeded = & "$PSScriptRoot/seed_demo_logs.ps1" -LokiUrl $LokiUrl -BaseTime $baseTime -RequestPrefix $prefix -PassThruCount
    $total += [int]$seeded
}
