...
import { formatCypherError } from './formatCypherError'

describe('formatCypherError', () => {
  it('formats a raw syntax error with a translated message body and location', () => {
    ...
    expect(formatCypherError(raw)).toBe(
  ...
  it('falls back to the category only when the message body does not match a known pattern', () => {
    ...
    expect(formatCypherError(raw)).toBe('제약 조건 위반으로 실패했습니다.')
  })

  it('falls back to a generic Korean label for unrecognized neo4j_code categories', () => {
    expect(
      formatCypherError(
        '{neo4j_code: Neo.ClientError.Statement.SomethingElse} {message: whatever}',
      ),
    ).toBe('쿼리 실행 오류로 실패했습니다.')
  })

  it('returns already human-readable Korean messages unchanged', () => {
    expect(formatCypherError('접속 오류가 발생했습니다.')).toBe('접속 오류가 발생했습니다.')
    expect(formatCypherError('스키마에 없는 Label: CAUSED_BY')).toBe(
      '스키마에 없는 Label: CAUSED_BY',
    )
  })

  it('returns strings that do not match the raw driver pattern unchanged', () => {
    expect(formatCypherError('syntax error at or near "NOT"')).toBe('syntax error at or near "NOT"')
  })
})
